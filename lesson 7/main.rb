...
class TextInterface
  ...
  def add_carriages
    ...
    size = get_int_choice
    carriage = create_carriage!("#{rand(1000..3000)}", size, train.type)
  ...
  def fill_carriage
    ...
    when :cargo
      puts "#{@@info} данный вагон имеет общий объем #{@carriages[index_carriage].space}, сейчас в нем свободно #{@carriages[index_carriage].free_space}"
      ...
      puts "#{@@done} в вагон '#{@carriages[index_carriage].number}' добавлен груз, осталось свободного места #{@carriages[index_carriage].free_space}"
    end
  rescue => e
    puts e.message
    retry
